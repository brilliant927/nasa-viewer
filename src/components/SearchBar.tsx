    ...
    <div className="flex justify-center gap-4 flex-wrap">
      <div className="flex  justify-center flex-wrap gap-4" >
        <div className="flex flex-col">
          <input className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 ${!validates.query ? 'border-red-600' : 'border-black'}`} type="text" value={query} placeholder="Query" onChange={handleChangeQuery} />
          <label className={`text-red-600 ${!validates.query ? 'block' : 'hidden'}`} >Invalid Query</label>
        </div>
        <div className="flex flex-col">
          <input className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 ${!validates.startAt ? 'border-red-400' : 'border-black'}`}  value={startAt} placeholder="Start Year" onChange={handleChangeStartAt} />
          <label className={`text-red-600 ${!validates.startAt ? 'block' : 'hidden'}`} >Invalid Start Year</label>
        </div>
        <div className="flex flex-col">
          <input className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 ${!validates.endAt ? 'border-red-400' : 'border-black'}`}  value={endAt} placeholder="End Year" onChange={handleChangeEndAt} />
          <label className={`text-red-600 ${!validates.endAt ? 'block' : 'hidden'}`} >Invalid End Year</label>
        </div>
      </div>
      <div>
      <button type="button" onClick={handleSearchButton} className="p-2.5 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
